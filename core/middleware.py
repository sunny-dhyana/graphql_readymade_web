  ...
  def resolve(self, next, root, info, **kwargs):
    ...
    for q in array_qry:
      ...
      if query:
        ...
        depth = parser.get_depth(mutation)

      if security.depth_exceeded(depth):
        raise GraphQLError('400 Bad Request: Query Depth Exceeded! Deep Recursion Attack Detected.')
...
class CostProtectionMiddleware(object):
  def resolve(self, next, root, info, **kwargs):
    ...
    if security.cost_exceeded(fields_requested):
      raise GraphQLError('400 Bad Request: Cost of Query is too high.')
...
class OpNameProtectionMiddleware(object):
  ...
  def resolve(self, next, root, info, **kwargs):
    ...
    if opname != 'No Operation' and not security.operation_name_allowed(opname):
      raise GraphQLError('400 Bad Request: Operation Name "{}" is not allowed.'.format(opname))
    ...
    if info.context.json is not None:
      ...
      for q in array_qry:
        ...
        if security.on_denylist(query):
          raise GraphQLError('400 Bad Request: Query is on the Deny List.')
...
class IntrospectionMiddleware(object):
  ...
  def resolve(self, next, root, info, **kwargs):
    ...
    if info.field_name.lower() in ['__schema']:
      raise GraphQLError('400 Bad Request: Introspection is Disabled')
...
class IGQLProtectionMiddleware(object):
  ...
  def resolve(self, next, root, info, **kwargs):
    if helpers.is_level_hard():
      raise GraphQLError('400 Bad Request: GraphiQL is disabled')

    cookie = request.cookies.get('env')
    if cookie and cookie == 'graphiql:enable':
      return next(root, info, **kwargs)

    raise GraphQLError('400 Bad Request: GraphiQL Access Rejected')
